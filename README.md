...
| SMB share | The ISO storage as `\\<hostname>\media`: guests read, `admin` writes, and the device is listed by the macOS Finder and the Windows Explorer -- see [docs/smb.md](docs/smb.md) |
...
smb/        the ksmbd init script and wsdd (Windows network discovery)
